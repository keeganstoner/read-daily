...
- **`january/january-NN.txt`** — the same readings as plain text, one file per day, plus `index.json`
  (per-day metadata: source URLs, word counts, how each reading was assembled).
...
| `generate.py`    | write the plain-text `january/` files |
...
Plain-text instead of EPUB: `python3 scrape/generate.py`.
